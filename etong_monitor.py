...
def load_config():
    """从外部 config.json 加载配置，覆盖硬编码默认值"""
    global SSO_USERNAME, SSO_PASSWORD, JWT_TOKEN, CTTICKET
    global WECOM_WEBHOOK, BARK_KEY, PUSHPLUS_TOKEN
    global LOW_BALANCE_THRESHOLD, CHECK_INTERVAL, DAILY_REPORT_HOUR, DAILY_REPORT_MINUTE, ALERT_COOLDOWN
    try:
        with open(CONFIG_FILE, 'r') as f:
            cfg = json.load(f)
        SSO_USERNAME = cfg.get("sso_username", SSO_USERNAME)
        SSO_PASSWORD = cfg.get("sso_password", SSO_PASSWORD)
        JWT_TOKEN = cfg.get("jwt_token", JWT_TOKEN)
        CTTICKET = cfg.get("ctticket", CTTICKET)
        WECOM_WEBHOOK = cfg.get("wecom_webhook", WECOM_WEBHOOK)
        BARK_KEY = cfg.get("bark_key", BARK_KEY)
        PUSHPLUS_TOKEN = cfg.get("pushplus_token", PUSHPLUS_TOKEN)
        LOW_BALANCE_THRESHOLD = cfg.get("low_balance_threshold", LOW_BALANCE_THRESHOLD)
        CHECK_INTERVAL = cfg.get("check_interval", CHECK_INTERVAL)
        DAILY_REPORT_HOUR = cfg.get("daily_report_hour", DAILY_REPORT_HOUR)
        DAILY_REPORT_MINUTE = cfg.get("daily_report_minute", DAILY_REPORT_MINUTE)
        ALERT_COOLDOWN = cfg.get("alert_cooldown", ALERT_COOLDOWN)
        print(f"✅ 已加载外部配置: {CONFIG_FILE}")
    except FileNotFoundError:
        print(f"ℹ️  未找到 {CONFIG_FILE}，使用脚本内默认配置")
    except Exception as e:
        print(f"⚠️  加载配置文件失败: {e}，使用脚本内默认配置")
...
def save_ctticket_to_config(cookies_dict):
    ...
    try:
        ...
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, 'r') as f:
                cfg = json.load(f)
        cfg["ctticket"] = ctticket
        with open(CONFIG_FILE, 'w') as f:
            json.dump(cfg, f, indent=2, ensure_ascii=False)
        log(f"💾 CTTICKET 已保存到 {CONFIG_FILE}")
    except Exception as e:
        log(f"⚠️  保存 CTTICKET 失败: {e}")


def save_jwt_to_config(jwt_token):
    """保存 JWT/etToken 到外部 config.json"""
    if not jwt_token:
        return
    try:
        cfg = {}
        ...
        with open(CONFIG_FILE, 'w') as f:
            json.dump(cfg, f, indent=2, ensure_ascii=False)
        log(f"💾 JWT_TOKEN 已保存到 {CONFIG_FILE}")
    except Exception as e:
...
def sso_login():
    ...
    try:
        ...
        if isinstance(resp_data, dict):
            ...
            # 保存服务器返回的完整 locatUrl，用于 2FA 后的重定向（包含 before=1&type=1）
            sso_locat_url = locat_url
        ...
        if need_2fa:
            ...
            # 重定向页面不含 etToken，仅用于获取 CTTICKET
            if not any("CTTICKET" in k.upper() for k in cookies):
                # 重定向后没有 CTTICKET，尝试访问 etong 首页
                log("🔄 重定向后未获取到 CTTICKET，尝试访问 etong 首页...")
...
def send_notification(title, message):
    ...
    if not sent:
        log("⚠️  未配置推送渠道或全部推送失败")
    return sent


def wecom_push(title, message):
    """企业微信推送：使用系统 curl 避免 curl_cffi 的 BoringSSL 兼容性问题"""
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if status_code == "200":
                log(f"📱 企业微信推送成功 (attempt {attempt}/{max_retries}, status=200)")
                return True
            else:
                log(f"⚠️  企业微信推送返回非200 (attempt {attempt}): {status_code}")
        except Exception as e:
            log(f"⚠️  企业微信推送异常 (attempt {attempt}/{max_retries}): {e}")
        if attempt < max_retries:
            time.sleep(3)
    log(f"❌ 企业微信推送 {max_retries} 次均失败")
    return False
