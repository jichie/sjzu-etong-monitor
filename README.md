...
- 🛡️ **SSO session cookies 持久化**：2FA 成功后保存 SSO 会话 cookies，后续登录复用，避免反复 2FA
- 🔧 **修复 APPCTTICKET _webreq 错误**：APPCTTICKET 不应带 `_webreq` 后缀，否则影响 token 有效性
- 🍪 在 verifyPhone 成功时和重定向后两次保存 cookies，确保信任标记不丢失：使用 verifyPhone 返回的 `before=2` locatUrl 完成设备注册
- 🔑 **保存 app_token**：验证成功时保存设备令牌，后续登录优先使用
- 📱 **2FA 验证成功推送**：SSO 重新认证成功后自动推送电量确认通知：SSO 重新认证成功后自动推送电量确认通知
