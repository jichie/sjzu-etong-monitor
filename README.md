...
- 💓 **心跳保活** - 验证通过后每 10 分钟自动刷新 session，不再需要反复 2FA
...
> 不再需要修改 `etong_monitor.py` 源码，所有配置统一在 `.env` 中管理。
...
`.env` 文件权限自动设为 600，不会被提交到 Git。

### 方式二：环境变量

支持 systemd `EnvironmentFile` 或 Docker 环境变量注入。
...
| `CTTICKET` | 从浏览器获取的 CTTICKET（可选，留空则自动获取） | 选填 |
| `SMS_SERVER_PORT` | 验证码接收服务端口，默认 8899 | 选填 |
| `SMS_CODE_TIMEOUT` | 验证码等待超时（秒），默认 300 | 选填 |
| `ALERT_COOLDOWN` | 告警冷却时间（秒），默认 21600（6小时） | 选填 |
...
为了避免 SSO 系统频繁触发二次验证，可以在 智慧建大App 中将部署脚本的设备设置为常用设备：
...
- 🔒 **凭据外置**：学号、密码、Webhook Key 等全部移至 `.env` 文件（权限 600），不再硬编码在源码中
- 🧹 **Git 历史清洗**：使用 `git filter-repo` 清除全部 48 个 commit 中的敏感信息
- 🔧 **移除 sudo 密码管道**：不再通过 `echo 密码 | sudo cp` 修改自身源码，改用 `config.json` 持久化 Token
- 📦 **新增 `.env` 配置模板**：配合 `python-dotenv` 自动加载，支持 systemd `EnvironmentFile`
- 🛡️ **`CAPTCHA_VERIFY_URLS` 启用**：图片验证码提交现在循环尝试所有候选接口
- ⏰ **修复日报时间 Bug**：`now.minute < min+59` → `now.minute >= min`
- 💓 **心跳线程修复**：使用模块级 `http_get` 保持一致 TLS 指纹，改用 `threading.Event`
- 🔄 **查询超时降级**：`curl_cffi` 请求超时时自动切换普通 `requests` 重试
- ⚡ **代码质量提升**：提取 `build_multipart()` 公共函数消除 5 处重复代码，添加类型标注，裸 `except` 改为记录异常
- 🏗️ **systemd 改进**：新增 `WorkingDirectory=/opt/etong`、`NoNewPrivileges`、`PrivateTmp`

### v9.3

- 💓 **心跳保活**：每 10 分钟自动访问 etong 首页刷新 session，SSO 令牌永不超时，彻底避免重复 2FA
- 🔧 **清理无效补丁**：移除导致验证码失效的错误修改，恢复原始验证码流程
- 📦 脚本不再依赖 `/opt/etong/config.json`（凭证硬编码），移除外部配置文件依赖
- 🧹 修复了 `save_ctticket_to_config` 函数权限错误
- 📝 优化 get_balance 认证状态追踪

### v9.1

- 🔧 **修复 CTTICKET 自动保存**：原正则表达式损坏导致新 CTTICKET 永远无法写回，改为外部 `config.json` 存储
- 🔧 **修复 2FA 后 etToken 未提取**：2FA 验证通过后补充访问 etong 首页提取 etToken，解决 `无效的access_token` 错误
- 🔧 **修复设备无法设为常用设备**：使用服务器返回的 `locatUrl`（`before=1&type=1`）替代硬编码的 `before=0&type=0`
- 📱 **修复企业微信推送失败**：`curl_cffi` 的 BoringSSL 与企业微信 API 不兼容（90% 失败率），推送改用系统 `curl` + 3 次重试
- 📦 **外部配置文件**：敏感凭据（SSO 密码、JWT、Webhook Key 等）移至 `config.json`（权限 600），脚本启动时自动加载
- ⚡ **优化认证优先级**：优先使用缓存 cookies，不再每次浪费请求尝试过期的 CTTICKET
- 🛡️ **systemd 服务改进**：`Restart=always` + `RestartSec=60` + 日志输出到文件
- 📝 裸 `except` 子句改为记录异常日志
